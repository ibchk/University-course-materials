...
class Order:
    ...
    def total_volume(self) -> int:
        ...
        vol = 0
        for i in self.order_items:
            vol += OrderItem(i.customer, i.name, i.quantity, i.one_item_volume).total_volume
        return vol
...
class OrderAggregator:
    ...
    def aggregate_order(self, customer: str, max_items_quantity: int, max_volume: int):
        ...
        items = []
        for i in self.order_items:
            if i.customer == customer and i.quantity <= max_items_quantity and i.one_item_volume <= max_volume:
                items.append(i)
                max_items_quantity -= i.quantity
                max_volume -= i.one_item_volume
        for i in items:
            for s in self.order_items:
                if i == s:
...
class ContainerAggregator:
    ...
    def prepare_containers(self, orders: tuple) -> dict:
        ...
        dict_cons = {}
        for i in orders:
            if i.total_volume > self.container_volume:
                self.not_used_orders.append(i)
            elif i.destination not in dict_cons:
                li = []
                cont = Container(self.container_volume, [i])
                li.append(cont)
                dict_cons[i.destination] = li
            else:
                for p in dict_cons[i.destination]:
                    if p.volume_left >= i.total_volume:
                        p.orders.append(i)
                        break
                    else:
                        con = Container(self.container_volume, [i])
                        dict_cons[i.destination].append(con)
                        break
        return dict_cons
